...
# 修改1：改为 utf-8编码。Global Configuration
with open("config-test.yml", "r", encoding="utf-8") as f:
    config = yaml.load(f, Loader=yaml.FullLoader)

# 修改2：参数中的替换
if "dataset" in config and "metric" in config:
    for key in ["train_path", "test_path", "output_path"]:
        if key in config:
            config[key] = config[key].format(
                dataset=config["dataset"], metric=config["metric"]
            )
...
def train_model(algorithm, train_data: np.ndarray, params):
    ...
    torch.save(
        model,
        f"model/{algorithm}_{params['epoch']}_{params['batch_size']}_{params['learning_rate']}.pt",
    )
    ...
    if "model_path" in params:
        if os.path.exists(params["model_path"]):
            ...
            raise FileNotFoundError(
                f"'{params['model_path']}' can not be found. Please check the model_path parameter in config.yaml"
            )
    else:
        model = get_model_LSTM(
            train_data,
        ...
        torch.save(
            model,
            f"model/LSTM_{params['epoch']}_{params['batch_size']}_{params['learning_rate']}.pt",
        )
...
def run_gru(train_data: np.ndarray, test_data: np.ndarray, params):
    if "model_path" in params:
        ...
        torch.save(
            model,
            f"model/GRU_{params['epoch']}_{params['batch_size']}_{params['learning_rate']}.pt",
        )
...
def run_mlstm(train_data: np.ndarray, test_data: np.ndarray, params):
    if "model_path" in params:
        ...
        torch.save(
            model,
            f"model/MLSTM_{params['epoch']}_{params['batch_size']}_{params['learning_rate']}.pt",
        )

    scores, dim_scores = get_prediction_MLSTM(
        model, test_data, params["seq_len"], params["modal"]
    )
...
def run_ae(train_data: np.ndarray, test_data: np.ndarray, params):
    if "model_path" in params:
        ...
        torch.save(
            model,
            f"model/AE_{params['epoch']}_{params['batch_size']}_{params['learning_rate']}.pt",
        )
...
def run_mmae(train_data: np.ndarray, test_data: np.ndarray, params):
    if "model_path" in params:
        ...
        torch.save(
            model,
            f"model/MMAE_{params['epoch']}_{params['batch_size']}_{params['learning_rate']}.pt",
        )

    scores, dim_scores = get_prediction_MMAE(
        model, test_data, params["seq_len"], params["modal"]
    )
    return scores
...
def run_vae(train_data: np.ndarray, test_data: np.ndarray, params):
    if "model_path" in params:
        ...
        torch.save(
            model,
            f"model/VAE_{params['epoch']}_{params['batch_size']}_{params['learning_rate']}.pt",
        )
...
if __name__ == "__main__":
    ...
    if "test_path" in config:
        ...
        if not multi_cases:
            ...
            test_info_ls = [
                (load_data(os.path.join(config["test_path"], i)), i)
