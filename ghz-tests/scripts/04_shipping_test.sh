...
setup_port_forward() {
    ...
    kubectl port-forward -n ${NAMESPACE} svc/shippingservice ${PORT}:${PORT} &
    PORT_FORWARD_PID=$!
...
main() {
    ...
    ITEMS_DATA='[{"product_id":"OLJCESPC7Z", "quantity":1}]'
    
    # 1. 测试 GetQuote 方法
    run_test "GetQuote" "{\"address\":${ADDRESS_DATA}, \"items\":${ITEMS_DATA}}" "获取运费报价"
    
    # 2. 测试 ShipOrder 方法
    run_test "ShipOrder" "{\"address\":${ADDRESS_DATA}, \"items\":${ITEMS_DATA}}" "处理订单配送"
    
    # 3. 测试不同负载下的性能 - 使用RPS负载控制
    echo -e "\n${YELLOW}==== 测试: ${SERVICE}.GetQuote - RPS限制 ====${NC}"
    ghz --insecure \
        --proto="${PROTO_FILE}" \
        --call="hipstershop.${SERVICE}.GetQuote" \
        -d "{\"address\":${ADDRESS_DATA}, \"items\":${ITEMS_DATA}}" \
        -n 2000 \
        --rps=100 \
        --connections=10 \
        --format=html \
        --output="${RESULTS_DIR}/${SERVICE}_GetQuote_rps_limit.html" \
        localhost:${PORT}
