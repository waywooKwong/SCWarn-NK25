...
setup_port_forward() {
    ...
    kubectl port-forward -n ${NAMESPACE} svc/paymentservice ${PORT}:${PORT} &
    PORT_FORWARD_PID=$!
...
main() {
    ...
    # 请求数据
    AMOUNT_DATA='{"currency_code":"USD", "units":100, "nanos":0}'
    ...
    # 1. 测试 Charge 方法
    run_test "Charge" "{\"amount\":${AMOUNT_DATA}, \"credit_card\":${CREDIT_CARD_DATA}}" "处理支付"
    
    # 2. 测试不同金额的支付
    AMOUNT_DATA_LARGE='{"currency_code":"USD", "units":9999, "nanos":990000000}'
    run_test "Charge" "{\"amount\":${AMOUNT_DATA_LARGE}, \"credit_card\":${CREDIT_CARD_DATA}}" "处理大金额支付"
    
    # 3. 测试异步请求模式
    echo -e "\n${YELLOW}==== 测试: ${SERVICE}.Charge - 异步请求 ====${NC}"
    ghz --insecure \
        --proto="${PROTO_FILE}" \
        --call="hipstershop.${SERVICE}.Charge" \
        -d "{\"amount\":${AMOUNT_DATA}, \"credit_card\":${CREDIT_CARD_DATA}}" \
        -n 1000 -c 50 -z 30s \
        --connections=10 \
        --async \
        --format=html \
        --output="${RESULTS_DIR}/${SERVICE}_Charge_async.html" \
        localhost:${PORT}
