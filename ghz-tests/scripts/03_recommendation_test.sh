...
setup_port_forward() {
    ...
    kubectl port-forward -n ${NAMESPACE} svc/recommendationservice ${PORT}:${PORT} &
    PORT_FORWARD_PID=$!
...
main() {
    ...
    # 1. 测试 ListRecommendations 方法 - 单个产品
    run_test "ListRecommendations" '{"user_id":"test-user", "product_ids":["OLJCESPC7Z"]}' "获取单个产品的推荐"
    
    # 2. 测试 ListRecommendations 方法 - 多个产品
    run_test "ListRecommendations" '{"user_id":"test-user", "product_ids":["OLJCESPC7Z", "66VCHSJNUP", "1YMWWN1N4O"]}' "获取多个产品的推荐"
    
    # 3. 测试持续时间设置（不同于默认的30秒）
    echo -e "\n${YELLOW}==== 测试: ${SERVICE}.ListRecommendations - 短时间高并发 ====${NC}"
    ghz --insecure \
        --proto="${PROTO_FILE}" \
        --call="hipstershop.${SERVICE}.ListRecommendations" \
        -d '{"user_id":"test-user", "product_ids":["OLJCESPC7Z"]}' \
        -n 2000 -c 100 -z 15s \
        --connections=10 \
        --format=html \
        --output="${RESULTS_DIR}/${SERVICE}_ListRecommendations_high_concurrency.html" \
        localhost:${PORT}
